...
const countryData = new TextDecoder().decode(await Deno.readFile('./countryList.json'));
const countryList = JSON.parse(countryData) as ICountry[];

// console.log(countryList);

let dataStream = new TextDecoder().decode(await Deno.readFile('./bankMap.json'));
let bankMap = JSON.parse(dataStream) as Record<string, Record<string, string>[]>;

let branch: Record<string, string[]>

async function loadBranch(countryCode: string, bankName: string) {
  const resp = await fetch("https://www.theswiftcodes.com/ajax/code-finder.ajax.php", {
    "headers": {
...
async function loadSwiftCode(countryCode: string, bankName: string, branchName: string) {
  const resp = await fetch("https://www.theswiftcodes.com/ajax/code-finder.ajax.php", {
    "headers": {
      "accept": "application/json, text/javascript, */*; q=0.01",
      "accept-language": "en-US,en;q=0.9",
    ...
    "referrer": "https://www.theswiftcodes.com/",
    "referrerPolicy": "no-referrer-when-downgrade",
    "body": `input=city&country=${countryCode}&bank=${encodeURI(bankName)}&city=${encodeURI(branchName)}`,
    "method": "POST",
    "mode": "cors",
    "credentials": "include"
  });
    ...
    .then(branches => {
      return Promise.all(
        branches.map(async (branch: R) => {
          const swiftCodeList = await loadSwiftCode(country, bank.value, branch.value);
          return {
            country,
            bank: bank.value,
            branch: branch.value,
...
let errorBuffer = new TextDecoder().decode(await Deno.readFile('./errors.json'));
...
for await (const country of countriesToWrite) {
  ...
  for await (const banks of _.chunk(bankMap[country], 5)) {
    try {
      const _list: R[] = await Promise.all(banks.map((bank: R) => getSwiftCodes(country, bank)));
  ...
  await Deno.writeFile(`dist/${country}.json`, new TextEncoder().encode(JSON.stringify(swiftCodeList)));
  await Deno.writeFile('errors.json', new TextEncoder().encode(JSON.stringify(errors)));
  remainingCountry--;
}
