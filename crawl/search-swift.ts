...
const countryDate = new TextDecoder().decode(await Deno.readFile('./countryList.json'));
const countryList = JSON.parse(countryDate).map((country: ICountry) => country.code!);
...
let errorBuffer = new TextDecoder().decode(await Deno.readFile('./errors2.json'));
...
for await (const country of countriesToWrite) {
  console.log(country);
  const buffer = new TextDecoder().decode(await Deno.readFile(`dist/${country}.json`));
  ...
  await Deno.writeFile(`swift/${country}.json`, new TextEncoder().encode(JSON.stringify(codes)));
  await Deno.writeFile('errors2.json', new TextEncoder().encode(JSON.stringify(errors)));
