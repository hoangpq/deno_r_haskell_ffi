import { cstr } from "../core/utils.ts";
...
let Cyber!: Deno.DynamicLibrary<typeof SYMBOLS>["symbols"];

try {
  Cyber = Deno.dlopen(
    new URL("../libs/libcyber.dylib", import.meta.url),
    SYMBOLS,
  ).symbols;

  const vm = Cyber.cyVmCreate();

  const script = `
import os 'os'

raylibPath = '/usr/local/Cellar/raylib/4.2.0/lib/libraylib.dylib'
...
`
  ...
  const val = Cyber.cyVmEval(vm, cstr(script), script.length);
  console.log(Cyber.cyValueAsI32(val));
